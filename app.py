...
with st.beta_expander("How it works:"):
    text = '''Selecting any subject/course will give you the results of number of best universities in top30 countries. Then after 
...
def university_location(Universityrank, university, lat, lon):
    ...
    with column111:
        ...
        st.info('Note: Here, the location might be inaccurate for some universities')
    with column222:
...
subjects_offered.remove('Politics & International Studies (incl Development Studies)')
...
if selected_subject:
    country_data = df.loc[df['subjects_offered'].str.contains(selected_subject)]
...
with Reach_us:
    st.write("Our linkedIn Profiles:")
...
with Project_repo:
    ...
    st.write("Please give a star :star2: if you liked our work")

st.markdown("<h4 style='text-align:center;'>Thank you so much for visiting us.Have a good day</h4>", unsafe_allow_html=True)